...
- Next credible SOTA direction: make a new stronger English retriever or local reranker available, then benchmark it through `scripts/run_nfcorpus_candidate.py`; agreement-only fusion, rank-score fusion, branch-depth, calibration, pooling, supervised feature fusion, nonlinear feature fusion, learned cascades, lexical-field changes, and the newly discovered BCE embedding have not closed the SOTA gap, so do not tune fusion weights on NFCorpus test qrels and do not weaken the target.
