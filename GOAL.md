...
- Best current local artifact: `artifacts/runs/bge_small_gbdt_regression_dense_prf_dev_selected_dev_score_fusion_20260608T214206Z.json`.
- Best current local score: `nDCG@10 = 0.369215383024794`, `Recall@100 = 0.33559899868294146`, 323 queries, 0 failures.
- Current gap to selected primary target: `0.10068461697520598` absolute `nDCG@10`.
- Current gap to official BEIR comparator: `0.015784616975206` absolute `nDCG@10`.
- Current best local method: second-stage dev-calibrated min-max score fusion over the previous train/dev graded-regression score-fusion anchor and a dev-selected BGE-small dense pseudo-relevance feedback branch; the underlying source set remains `Xenova/bge-small-en-v1.5` ONNX with both CLS and mean pooling, `Xenova/all-MiniLM-L6-v2` ONNX, and BM25.
...
- Latest measured deep-pool graded-regression branch: `artifacts/runs/bge_small_dual_pool_xenova_minilm_bm25_deep_train_dev_gbdt_regression_dev_score_fusion_20260608T202346Z.json` trained the graded GBDT regressor on `train`, selected two-ranker fusion weights on `dev`, used `branch_k = 300`, then scored `nDCG@10 = 0.363902495492805`, `Recall@100 = 0.33490230529221543` on `test`; it is rejected because it loses `0.003680547215140606` primary `nDCG@10` versus the current best despite improving `Recall@100` by `0.0020237225884362497`.
- Latest measured local domain-supervised checkpoint branch: `artifacts/runs/scifact_finetuned_minilm_20260608T203922Z.json` benchmarked a complete local SentenceTransformer checkpoint fine-tuned from all-MiniLM-L6-v2 on SciFact-style biomedical pairs, then scored `nDCG@10 = 0.3113948750306552`, `Recall@100 = 0.30306730089962974` on `test`; it is rejected because it loses `0.0561881676772904` primary `nDCG@10` and `0.029811281804149437` `Recall@100` versus the current best.
- Latest measured no-test-leak checkpoint-selection branch: `artifacts/runs/scifact_dev_selected_minilm_20260608T205002Z.json` selected `checkpoint-9` from six local SciFact-finetuned MiniLM checkpoints using NFCorpus `dev` qrels only, then scored `nDCG@10 = 0.316500938704734`, `Recall@100 = 0.31019343221102363` on `test`; it is rejected because it loses `0.051082104003211615` primary `nDCG@10` and `0.022685150492755546` `Recall@100` versus the current best despite a tiny `0.0004997209025133786` `nDCG@10` gain over direct MiniLM.
- Latest measured current-best plus no-test-leak SciFact checkpoint-selection fusion: `artifacts/runs/bge_small_gbdt_regression_scifact_dev_selected_dev_score_fusion_20260608T210404Z.json` dev-calibrated the current graded-regression score-fusion anchor against the dev-selected SciFact checkpoint branch, selected `current_best_primary = 0.5` and `scifact_secondary = 0.0`, then scored `nDCG@10 = 0.3675830427079456`, `Recall@100 = 0.33050103876645803` on `test`; it is not promoted because it ties the current best on primary `nDCG@10` and loses `0.002377543937321147` `Recall@100`.
- Latest measured dense pseudo-relevance feedback branch: `artifacts/runs/bge_small_en_onnx_dense_prf_20260608T212303Z.json` used qrels-free Rocchio-style query-vector feedback over BGE-small ONNX top documents, then scored `nDCG@10 = 0.3425690018844359`, `Recall@100 = 0.3105355971312376` on `test`; it is rejected because it loses `0.02501404082350972` primary `nDCG@10` and `0.022342985572541607` `Recall@100` versus the current best despite improving direct BGE-small ONNX by `0.0003240370649028601` `nDCG@10`.
- Latest measured no-test-leak dense PRF parameter-selection branch: `artifacts/runs/bge_small_en_onnx_dense_prf_dev_selected_20260608T213136Z.json` selected `feedback_docs = 3` and `feedback_weight = 0.5` on NFCorpus `dev`, then scored `nDCG@10 = 0.34933673642384316`, `Recall@100 = 0.3180000296586443` on `test`; it is rejected because it loses `0.018246306284102454` primary `nDCG@10` and `0.014878553045134857` `Recall@100` versus the current best despite improving direct BGE-small ONNX by `0.007091771604310126` `nDCG@10`.
- Latest promoted current-best plus no-test-leak dense PRF complement fusion: `artifacts/runs/bge_small_gbdt_regression_dense_prf_dev_selected_dev_score_fusion_20260608T214206Z.json` dev-calibrated the previous graded-regression score-fusion anchor against the dev-selected dense PRF branch, selected `current_best_primary = 2.0` and `dense_prf_secondary = 0.5` on `dev`, then scored `nDCG@10 = 0.369215383024794`, `Recall@100 = 0.33559899868294146` on `test`; it improves the previous best by `0.0016323403168483908` primary `nDCG@10` and `0.002720415979162283` `Recall@100`, but remains far below SOTA.
