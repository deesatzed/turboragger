...
- The branch uses Rocchio-style pseudo-relevance feedback over the initial BGE-small ONNX top documents.
- It does not use NFCorpus `train`, `dev`, or `test` qrels for feedback selection or parameter tuning.
- Test scoring produced `nDCG@10 = 0.3425690018844359`, below the current best `0.3675830427079456`.
- It improves direct `bge_small_en_onnx` by only `0.0003240370649028601` `nDCG@10`.
- It lowers direct BGE-small `Recall@100` from `0.3120478599231725` to `0.3105355971312376`.

Consequences:

- New helper: `dense_prf_query_vector`.
- New retriever: `DensePrfRetriever`.
- New candidate: `bge_small_en_onnx_dense_prf`.
- Run artifact: `artifacts/runs/bge_small_en_onnx_dense_prf_20260608T212303Z.json`.
- Current best remains `bge_small_dual_pool_xenova_minilm_bm25_train_dev_gbdt_regression_dev_score_fusion`.
- Fixed PRF over the same BGE-small signal is now covered; future query expansion should be train/dev-selected, generated by a stronger model, or paired with a stronger retriever.
